...
public class MessageStorageSQLMapper {
  ...
  private static final String MAPPER_PACKAGE =
      MessageStorageSQLMapper.class.getPackageName() + ".sql";
  ...
  public MessageStorageSQLMapper(final JdbcTemplate jdbcTemplate)
    ...
    try (var connection = Objects.requireNonNull(jdbcTemplate.getDataSource()).getConnection()) {
      ...
      this.sqlProperties =
          (MessageStorageSQLProperties)
              Class.forName(sqlPropertiesClassName).getDeclaredConstructor().newInstance();
    }
  }
  ...
  public void insert(MessageModel model) {
    var result =
        jdbcTemplate.update(
            sqlProperties.insertForMessageLog(),
            model.getFormerId(),
            model.getTopic(),
            model.getHashKey(),
            model.getCreateTime(),
